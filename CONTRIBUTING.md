...
このプロジェクトが Azure 上で扱うリソースは、次の 2 種類に限定されます。

- Cloud Custodian のポリシーを格納する **Blob Storage**
- Cloud Custodian による Azure リソース操作（読み取り / 変更）

開発時にアプリケーションを Azure Container Apps 上で動かすことは前提としておらず、ローカル環境や CI から Azure にアクセスする形で開発・検証を行います。

#### 1. Azure CLI でサインイン

1. [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) をインストール
2. `az login --tenant <TENANT_ID>` で対象テナントにサインイン
3. `az account set --subscription <SUBSCRIPTION_ID>` で既定サブスクリプションを切り替え

`DefaultAzureCredential` はサインイン済みの Azure CLI セッション（`AzureCliCredential`）を自動取得するため、追加のトークン設定は不要です ([ローカル開発での認証](https://learn.microsoft.com/azure/developer/python/sdk/authentication/local-development-dev-accounts#3---sign-in-to-azure-using-the-azure-cli,-azure-powershell,-azure-developer-cli,-or-in-a-browser))。

#### 2. サービス プリンシパルを使用する場合

CI や限定権限での検証が必要な場合は、以下でサービス プリンシパルを作成します:
...
発行された `appId` / `tenant` / `password` を `.env` に保存し、`python-dotenv` で読み込むと `DefaultAzureCredential` が `EnvironmentCredential` を経由して利用します ([サービス プリンシパルでのローカル開発](https://learn.microsoft.com/azure/developer/python/sdk/authentication/local-development-service-principal#set-local-development-environment-variables))。リポジトリには雛形として `.env.example` を用意しているのでコピーして値を埋めてください。
...
C7N_POLICY_FILE=${PWD}/examples/policies/scheduled-policy.yml
```
...
```bash
# ポリシー実行 (ローカル YAML)
c7n-azure-runner run-policy \
        --policy-file $C7N_POLICY_FILE \
        --subscription-id $AZURE_SUBSCRIPTION_ID

# Storage Queue イベントの検証
c7n-azure-runner run-event \
        --event-data @tests/fixtures/event.json \
        --policy-uri https://<storage>.blob.core.windows.net/policies/
```

#### 4. VS Code からのデバッグ実行

VS Code では、`.vscode/launch.json` に定義した構成を使って `c7n_azure_container_apps.cli` モジュールを直接デバッグ実行できます。

- `.env` に設定した `AZURE_SUBSCRIPTION_ID` や `C7N_POLICY_FILE` などの環境変数は、`envFile: "${workspaceFolder}/.env"` を通じてデバッグ構成からそのまま利用できます。
- 代表的な構成（`run-policy` / `run-event`）はすでに `.vscode/launch.json` に用意されているため、VS Code の「実行とデバッグ」ビューから選択してそのまま利用できます。
