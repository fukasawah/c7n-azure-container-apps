...
デバッグ時は `python -m pdb -m c7n_azure_container_apps.cli run-policy ...` でブレークポイント実行、もしくは VS Code の `launch.json` から `module` を `c7n_azure_container_apps.cli` に設定してください。テストは `pytest -k policy_executor -vv` のように対象を絞ることで再現が容易になります。
