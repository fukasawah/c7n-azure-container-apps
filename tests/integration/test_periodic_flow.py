...
class TestPeriodicFlow:
    ...
    def test_filter_periodic_from_mixed_policies(self):
        ...
        # Act - cli.py と同等のフィルタ
        filtered = [
            p for p in all_policies if p.data.get("mode", {}).get("type") == "container-periodic"
        ]
        filtered_collection = FakePolicyCollection(policies=filtered)

        policy_executor.execute_policies(filtered_collection)
    ...
    def test_partial_failure_continues_execution(self):
        ...
        policies = FakePolicyCollection(policies=[success_policy, failing_policy, another_success])
...
class TestMixedModeExecution:
    ...
    def test_execute_policies_with_event_calls_push(self):
        ...
        policy_executor.execute_policies(policies, event=event)
